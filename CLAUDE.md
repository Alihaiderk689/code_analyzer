...
- The frontend must call `GET /api/auth/csrf/` once on boot (`AuthContext.jsx` does this via `primeCsrf()` in `api.js`) before any mutating request — otherwise every POST/PATCH/DELETE 403s with "CSRF token missing." `primeCsrf()` reads the token from that endpoint's **response body** (`{"csrfToken": ...}`) and caches it in memory for `X-CSRFToken`, falling back to parsing the JS-readable `csrftoken` cookie only if the body path is unavailable — the body path is what makes this work even when frontend and backend aren't on subdomains of one registrable domain, since `document.cookie` can never read a cookie set by a genuinely unrelated domain.
- `POST /api/auth/refresh/` reads the refresh token from its cookie only, never the request body.
- `AUTH_COOKIE_SECURE`/`AUTH_COOKIE_SAMESITE` in `config/settings.py` are `Lax`+non-secure in dev, `None`+`Secure` in production — required for the httpOnly JWT cookies to flow cross-site at all in production, regardless of domain relatedness.
